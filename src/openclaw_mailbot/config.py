...
class Config:
    ...
    def load(cls, path: Path | str | None = None) -> "Config":
        ...
        ``POP3_PASSWORD`` is ignored in this slice because POP3 integration is
        not yet built. ``MAILBOT_DATA_DIR`` overrides any data_dir value from
        the INI file.
        """
        parser = ConfigParser()
        if path:
        ...
        return cls(
            ...
            webhook_url=parser.get("forwarder", "webhook_url", fallback=""),
            webhook_timeout_seconds=parser.getfloat(
                "forwarder", "timeout_seconds", fallback=30.0
            ),
        )
